...
// Generate mock bolão data for Dia de Sorte with realistic financial calculations
// Dia de Sorte: R$ 5.00 per game
const generateMegaSenaBoloes = () => {
  const boloes = [];
  const dezenaOptions = [50]; // Dia de Sorte sempre 50 dezenas
  const prizeRanges = [45, 55, 72, 85, 95, 120, 150, 180, 210, 250, 300]; // Prêmios em milhões
  
  // Base concurso number for Dia de Sorte (realistic)
  const baseConcurso = 2850;
  
  // Add 3 special Dia de Sorte da Independência bolões
  for (let i = 1; i <= 3; i++) {
    const dezenas = dezenaOptions[Math.floor(Math.random() * dezenaOptions.length)];
    const fillPercentage = Math.floor(Math.random() * 30) + 60;
    const prizeValue = 550; // Dia de Sorte da Independência sempre ~550M
    
    // Jogos: 800-1500 para Dia de Sorte da Independência
    const jogosGerados = 800 + Math.floor(Math.random() * 700);
    
    // Valor do bolão = jogos * R$ 5.00
    const bolaoValue = jogosGerados * CUSTO_POR_JOGO.dia-de-sorte;
    ...
    // Dia de Sorte da Independência has teimosinha by default (special)
    const teimosinha = {
      enabled: true,
      mode: "fixo" as const,
      concursos: 1
...
// Generate random game numbers for Dia de Sorte (60 numbers, select 6-20 dezenas)
...
const GamesModal = ({ isOpen, onClose, bolao, lotteryType }: GamesModalProps) => {
  ...
  const totalNumbers = 60; // Dia de Sorte has 60 numbers
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl bg-bolao-card border-bolao-card-border text-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gradient-to-r from-yellow-400/15 via-green-500/10 to-yellow-400/15 border border-yellow-400/30 rounded-lg p-4 mb-4">
        ...
        <div className="flex gap-2 mb-4">
          {games.map((_, index) => (
            <Button
              ...
              className={`flex-1 ${
                activeTab === index 
                  ? "bg-pink-600 hover:bg-yellow-400 text-white" 
              ...
              return (
                <div
                  ...
                  className={`aspect-square flex items-center justify-center rounded-lg text-sm font-bold transition-all ${
                    isSelected 
                      ? "bg-yellow-400 text-white shadow-lg shadow-yellow-400/30 scale-105" 
        ...
        <div className="mt-4 p-3 bg-bolao-dark/30 rounded-lg border border-bolao-card-border">
          ...
          <div className="flex flex-wrap gap-2">
            {games[activeTab].map((num) => (
              <span key={num} className="px-3 py-1 bg-yellow-400/20 text-pink-300 rounded-full text-sm font-bold border border-yellow-400/30">
                {String(num).padStart(2, "0")}
              </span>
            ))}
          </div>
        ...
        <div className="flex justify-end mt-4">
          <Button onClick={onClose} className="bg-pink-600 hover:bg-yellow-400 text-white">
            Fechar
          </Button>
        </div>
      </DialogContent>
...
const BolaoCard = ({ bolao, onViewGames, onParticipate }: BolaoCardProps) => {
  ...
  return (
    <Card className={`relative p-5 bg-gradient-to-br from-bolao-card/80 to-bolao-card/40 border-bolao-card-border transition-all duration-300 card-hover group overflow-hidden ${
      bolao.isVirada ? "border-2 border-yellow-500/70 hover:border-yellow-400" : "hover:border-yellow-400/50"
    }`}>
      ...
      <div className={`absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-100 transition-opacity ${
        bolao.isVirada ? "from-yellow-500/10 to-transparent" : "from-yellow-400/5 to-transparent"
      }`} />
      ...
      <div className="relative z-10">
          ...
          <div className="flex items-center gap-2">
            <Badge className={`font-semibold ${
              bolao.isVirada 
                ...
                : "bg-yellow-400/20 text-pink-300 border-yellow-400/30"
            }`}>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className={`p-2.5 rounded-xl border ${bolao.isVirada ? "bg-gradient-to-br from-orange-500/30 to-amber-600/20 border-orange-500/30" : "bg-gradient-to-br from-yellow-400/30 to-pink-600/20 border-yellow-400/30"}`}>
