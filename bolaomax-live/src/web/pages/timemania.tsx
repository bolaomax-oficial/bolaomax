      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            ...
            <Link to="/timemania" className="text-sm font-semibold text-green-400">
              Timemania
            </Link>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden py-4 border-t border-bolao-card-border">
            <nav className="flex flex-col gap-2">
              ...
              <Link to="/timemania" className="px-4 py-2 text-sm font-semibold text-green-400 bg-green-400/10 rounded-lg">
                Timemania
              </Link>
...
// Generate mock bolão data for Timemania with realistic financial calculations
// Timemania: R$ 5.00 per game
const generateMegaSenaBoloes = () => {
  const boloes = [];
  const dezenaOptions = [50]; // Timemania sempre 50 dezenas
  const prizeRanges = [45, 55, 72, 85, 95, 120, 150, 180, 210, 250, 300]; // Prêmios em milhões
  
  // Base concurso number for Timemania (realistic)
  const baseConcurso = 2850;
  
  // Add 3 special Timemania da Independência bolões
  for (let i = 1; i <= 3; i++) {
    const dezenas = dezenaOptions[Math.floor(Math.random() * dezenaOptions.length)];
    const fillPercentage = Math.floor(Math.random() * 30) + 60;
    const prizeValue = 550; // Timemania da Independência sempre ~550M
    
    // Jogos: 800-1500 para Timemania da Independência
    const jogosGerados = 800 + Math.floor(Math.random() * 700);
    
    // Valor do bolão = jogos * R$ 5.00
    const bolaoValue = jogosGerados * CUSTO_POR_JOGO.timemania;
    ...
    // Timemania da Independência has teimosinha by default (special)
    const teimosinha = {
      enabled: true,
      mode: "fixo" as const,
      concursos: 1
...
// Generate random game numbers for Timemania (60 numbers, select 6-20 dezenas)
...
const GamesModal = ({ isOpen, onClose, bolao, lotteryType }: GamesModalProps) => {
  ...
  const totalNumbers = 60; // Timemania has 60 numbers
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl bg-bolao-card border-bolao-card-border text-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gradient-to-r from-green-400/15 via-green-500/10 to-green-400/15 border border-green-400/30 rounded-lg p-4 mb-4">
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-lg bg-green-400/20 flex-shrink-0">
        ...
        <div className="flex gap-2 mb-4">
          {games.map((_, index) => (
            <Button
              ...
              className={`flex-1 ${
                activeTab === index 
                  ? "bg-pink-600 hover:bg-green-400 text-white" 
                  : "border-bolao-card-border hover:border-green-400/50"
              }`}
            >
              Jogo {index + 1}
            </Button>
        ...
        <div className="mt-4 p-3 bg-bolao-dark/30 rounded-lg border border-bolao-card-border">
          ...
          <div className="flex flex-wrap gap-2">
            {games[activeTab].map((num) => (
              <span key={num} className="px-3 py-1 bg-green-400/20 text-pink-300 rounded-full text-sm font-bold border border-green-400/30">
                {String(num).padStart(2, "0")}
              </span>
            ))}
          </div>
        ...
        <div className="flex justify-end mt-4">
          <Button onClick={onClose} className="bg-pink-600 hover:bg-green-400 text-white">
            Fechar
          </Button>
        </div>
      </DialogContent>
...
const BolaoCard = ({ bolao, onViewGames, onParticipate }: BolaoCardProps) => {
  ...
  return (
    <Card className={`relative p-5 bg-gradient-to-br from-bolao-card/80 to-bolao-card/40 border-bolao-card-border transition-all duration-300 card-hover group overflow-hidden ${
      bolao.isVirada ? "border-2 border-green-500/70 hover:border-green-400" : "hover:border-green-400/50"
    }`}>
      ...
      <div className={`absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-100 transition-opacity ${
        bolao.isVirada ? "from-green-500/10 to-transparent" : "from-green-400/5 to-transparent"
      }`} />
      ...
      <div className="relative z-10">
          ...
          <div className="flex items-center gap-2">
            <Badge className={`font-semibold ${
              bolao.isVirada 
                ...
                : "bg-green-400/20 text-pink-300 border-green-400/30"
            }`}>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className={`p-2.5 rounded-xl border ${bolao.isVirada ? "bg-gradient-to-br from-orange-500/30 to-amber-600/20 border-orange-500/30" : "bg-gradient-to-br from-green-400/30 to-pink-600/20 border-green-400/30"}`}>
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 bg-bolao-card border border-bolao-card-border rounded-lg text-sm font-medium hover:border-green-400/50 transition-colors min-w-[160px] justify-between"
      >
          ...
          <div className="absolute top-full left-0 mt-1 w-full bg-bolao-card border border-bolao-card-border rounded-lg shadow-xl z-20 py-1 max-h-60 overflow-auto">
            {options.map((option) => (
              <button
                ...
                className={`w-full px-4 py-2 text-sm text-left hover:bg-green-400/20 transition-colors ${
                  value === option ? "text-green-400 bg-green-400/10" : "text-muted-foreground"
                }`}
              >
                {option}
              </button>
...
const ViradaBanner = () => {
  ...
  return (
    <div className="relative mb-8 p-6 rounded-2xl bg-gradient-to-r from-orange-600/20 via-amber-500/20 to-orange-600/20 border border-orange-500/30 overflow-hidden">
      {/* Animated background */}
      <div className="absolute inset-0 bg-gradient-to-r from-green-500/10 via-transparent to-amber-500/10 animate-pulse" />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-xl md:text-2xl font-extrabold text-white">Timemania da Independência 2026</h2>
...
export default function Timemania() {
  ...
  const handleParticipate = (bolao: {
    ...
    const pendingBolao: PendingBolao = {
      ...
      name: bolao.codigoBolao || `Timemania ${bolao.dezenas}d`,
      type: "Timemania",
  ...
  return (
    <div className="min-h-screen bg-bolao-dark text-white">
      <SEOHead 
        ...
        description="Bolões da Timemania com os maiores prêmios do Brasil. Timemania da Independência disponível! Participe com cotas acessíveis."
        keywords={["bolão timemania", "timemania online", "time do coração", "apostas timemania", "sorteio timemania"]}
        pageType="lottery"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-3 rounded-xl bg-gradient-to-br from-green-600 to-green-800">
          ...
          <p className="text-muted-foreground ml-[60px]">
            Sorteios às terças, quintas e sábados • 80 números, 10 dezenas • Escolha seu Time do Coração!
          </p>
        </div>
