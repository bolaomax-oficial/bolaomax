      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            ...
            <Link to="/lotomania" className="text-sm font-semibold text-pink-400">
              Lotomania
            </Link>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden py-4 border-t border-bolao-card-border">
            <nav className="flex flex-col gap-2">
              ...
              <Link to="/lotomania" className="px-4 py-2 text-sm font-semibold text-pink-400 bg-pink-500/10 rounded-lg">
                Lotomania
              </Link>
...
// Generate mock bolão data for Lotomania with realistic financial calculations
// Lotomania: R$ 5.00 per game
const generateMegaSenaBoloes = () => {
  const boloes = [];
  const dezenaOptions = [50]; // Lotomania sempre 50 dezenas
  const prizeRanges = [45, 55, 72, 85, 95, 120, 150, 180, 210, 250, 300]; // Prêmios em milhões
  
  // Base concurso number for Lotomania (realistic)
  const baseConcurso = 2850;
  
  // Add 3 special Lotomania da Independência bolões
  for (let i = 1; i <= 3; i++) {
    const dezenas = dezenaOptions[Math.floor(Math.random() * dezenaOptions.length)];
    const fillPercentage = Math.floor(Math.random() * 30) + 60;
    const prizeValue = 550; // Lotomania da Independência sempre ~550M
    
    // Jogos: 800-1500 para Lotomania da Independência
    const jogosGerados = 800 + Math.floor(Math.random() * 700);
    
    // Valor do bolão = jogos * R$ 5.00
    const bolaoValue = jogosGerados * CUSTO_POR_JOGO.lotomania;
    ...
    // Lotomania da Independência has teimosinha by default (special)
    const teimosinha = {
      enabled: true,
      mode: "fixo" as const,
      concursos: 1
...
// Generate random game numbers for Lotomania (100 numbers: 00-99, select 50 dezenas)
const generateLotomaniaGameNumbers = (dezenas: number = 50) => {
...
const GamesModal = ({ isOpen, onClose, bolao, lotteryType }: GamesModalProps) => {
  ...
  const games = Array.from({ length: Math.min(numGames, 3) }, () => 
    generateLotomaniaGameNumbers(bolao.dezenas)
  );
  
  const totalNumbers = 100; // Lotomania has 100 numbers (00-99)
      ...
      <DialogContent className="sm:max-w-2xl bg-bolao-card border-bolao-card-border text-white max-h-[90vh] overflow-y-auto">
          ...
          <DialogTitle className="flex items-center gap-3 text-xl">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-normal mt-1">
                {bolao.dezenas} números de 00-99 • {numGames} {numGames === 1 ? "jogo" : "jogos"} de loteria
              </p>
            </div>
          </DialogTitle>
        </DialogHeader>
        
        {/* Card Summary - Lotomania specific */}
        <div className="bg-gradient-to-r from-pink-500/15 via-green-500/10 to-pink-500/15 border border-pink-500/30 rounded-lg p-4 mb-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-pink-200 mb-1">
                📊 Estrutura deste Card - Lotomania
              </p>
              <p className="text-sm text-pink-100/80">
                ...
                cada um com <strong className="text-pink-300">{bolao.dezenas} números</strong> selecionados do universo de <strong className="text-pink-300">00 a 99</strong>.
              </p>
              <p className="text-xs text-pink-200/60 mt-2">
                💡 Na Lotomania você escolhe 50 números e ganha acertando 20, 19, 18, 17, 16, 15 ou até 0 números!
              </p>
            </div>
          </div>
        </div>
            ...
            <Button
              ...
              className={`flex-1 ${
                activeTab === index 
                  ...
                  : "border-bolao-card-border hover:border-pink-500/50"
              }`}
            >
              Jogo {index + 1}
            </Button>
        ...
        <div className="mt-4 p-3 bg-bolao-dark/30 rounded-lg border border-bolao-card-border">
          <p className="text-sm text-muted-foreground mb-2">
            Números selecionados no Jogo {activeTab + 1} ({games[activeTab].length} de 100):
          </p>
          <div className="flex flex-wrap gap-2">
            {games[activeTab].map((num) => (
              <span key={num} className="px-3 py-1 bg-pink-500/20 text-pink-300 rounded-full text-sm font-bold border border-pink-500/30">
                {String(num).padStart(2, "0")}
              </span>
            ))}
          </div>
        ...
        <div className="flex justify-end mt-4">
          <Button onClick={onClose} className="bg-pink-600 hover:bg-pink-500 text-white">
            Fechar
          </Button>
        </div>
      </DialogContent>
...
const BolaoCard = ({ bolao, onViewGames, onParticipate }: BolaoCardProps) => {
  ...
  return (
    <Card className={`relative p-5 bg-gradient-to-br from-bolao-card/80 to-bolao-card/40 transition-all duration-300 card-hover group overflow-hidden ${
      bolao.isVirada 
        ? "border-2 border-orange-500/70 hover:border-orange-400 shadow-lg shadow-orange-500/10" 
        : "border-2 border-orange-500/60 hover:border-orange-400/90 shadow-md shadow-orange-500/8"
    }`}>
      {/* Virada special badge */}
      {bolao.isVirada && (
        <div className="absolute top-0 right-0">
          <div className="bg-gradient-to-r from-orange-600 to-amber-500 text-white text-[10px] font-bold px-3 py-1 rounded-bl-lg flex items-center gap-1 shadow-lg">
            <Star className="w-3 h-3 fill-current" />
            LOTOMANIA DA INDEPENDÊNCIA
          </div>
        </div>
      )}
      
      {/* Glow effect on hover - apenas nas bordas, mais sutil */}
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none`}>
        <div className={`absolute inset-0 rounded-lg ${
          bolao.isVirada 
            ? "shadow-[inset_0_0_20px_rgba(249,115,22,0.2)]" 
            : "shadow-[inset_0_0_15px_rgba(249,115,22,0.15)]"
        }`}></div>
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="mb-3">
          <TermometroAgressividade 
            nivel={calcularNivelAgressividade(bolao.jogosGerados, bolao.dezenas, "lotomania")}
            colorScheme={bolao.isVirada ? "orange" : "pink"}
          />
        </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className={`p-2.5 rounded-xl border ${bolao.isVirada ? "bg-gradient-to-br from-orange-500/30 to-amber-600/20 border-orange-500/30" : "bg-gradient-to-br from-pink-500/30 to-pink-600/20 border-pink-500/30"}`}>
        ...
        <div className="mt-4 flex items-center justify-center gap-3">
          ...
          <div className={`flex items-center gap-1 bg-bolao-dark/50 rounded-lg border ${
            bolao.isVirada ? "border-orange-500/30" : "border-pink-500/30"
          }`}>
            <Button
              variant="ghost"
              size="sm"
              ...
              className={`h-8 w-8 p-0 rounded-l-lg ${
                bolao.isVirada ? "hover:bg-orange-500/20" : "hover:bg-pink-500/20"
              }`}
            >
          ...
          <Button 
            ...
            className={`flex-1 font-semibold gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${
              bolao.isVirada 
                ...
                : "border-pink-500/50 text-pink-300 hover:bg-pink-500/10 hover:border-pink-400"
            }`}
          >
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 bg-bolao-card border border-bolao-card-border rounded-lg text-sm font-medium hover:border-pink-500/50 transition-colors min-w-[160px] justify-between"
      >
          ...
          <div className="absolute top-full left-0 mt-1 w-full bg-bolao-card border border-bolao-card-border rounded-lg shadow-xl z-20 py-1 max-h-60 overflow-auto">
            {options.map((option) => (
              <button
                ...
                className={`w-full px-4 py-2 text-sm text-left hover:bg-pink-500/20 transition-colors ${
                  value === option ? "text-pink-400 bg-pink-500/10" : "text-muted-foreground"
                }`}
              >
                {option}
              </button>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-xl md:text-2xl font-extrabold text-white">Lotomania da Independência 2026</h2>
...
export default function Lotomania() {
  const [apostaEspelhoFilter, setApostaEspelhoFilter] = useState<boolean | null>(null);
  const [teimosinhaModeFilter, setTeimosinhaModeFilter] = useState<string | null>(null);
  ...
  const handleParticipate = (bolao: {
    ...
    const pendingBolao: PendingBolao = {
      ...
      name: bolao.codigoBolao || `Lotomania ${bolao.dezenas}d`,
      type: "Lotomania",
  ...
  const filteredBoloes = boloes.filter((bolao) => {
    // Filtro de Aposta Espelho
    if (apostaEspelhoFilter !== null) {
      const hasApostaEspelho = bolao.apostaEspelho?.enabled === true;
      if (apostaEspelhoFilter !== hasApostaEspelho) return false;
    }
    
    // Filtro de Teimosinha
    if (teimosinhaModeFilter !== null) {
      if (!bolao.teimosinha?.enabled) return false;
      if (teimosinhaModeFilter === "ate_ganhar" && bolao.teimosinha.mode !== "ate_ganhar") return false;
      if (teimosinhaModeFilter === "quantidade" && bolao.teimosinha.mode === "ate_ganhar") return false;
    }
  ...
  return (
    <div className="min-h-screen bg-bolao-dark text-white">
      <SEOHead 
        title="Bolões Lotomania"
        description="Bolões da Lotomania com os maiores prêmios do Brasil. Lotomania da Independência disponível! Participe com cotas acessíveis."
        keywords={["bolão mega-sena", "mega-sena online", "mega da virada", "apostas mega-sena", "sorteio mega"]}
        pageType="lottery"
        lotteryType="Lotomania"
        canonicalUrl="https://bolaomax.com.br/lotomania"
        breadcrumbs={[
          { name: "Home", url: "/" },
          { name: "Bolões Lotomania", url: "/lotomania" }
        ]}
      />
      <Header />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-3 rounded-xl bg-gradient-to-br from-pink-600 to-pink-800">
              <Trophy className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl sm:text-4xl font-extrabold">Bolões Lotomania</h1>
            </div>
          </div>
          <p className="text-muted-foreground ml-[60px]">
            Sorteios às quartas e sábados • 60 números, 6 dezenas
          </p>
        </div>

        {/* Filters Section */}
        <div className="mb-8 space-y-4">
          {/* Aposta Espelho Filter */}
          <div>
            <p className="text-sm font-medium text-muted-foreground mb-3">Filtrar por Aposta Espelho:</p>
            <div className="flex flex-wrap gap-2">
              <Button
                variant={apostaEspelhoFilter === null ? "default" : "outline"}
                onClick={() => setApostaEspelhoFilter(null)}
                className={`${
                  apostaEspelhoFilter === null 
                    ? "bg-orange-600 hover:bg-orange-500 text-white" 
                    : "border-bolao-card-border hover:border-orange-500/50"
                }`}
              >
                Todos
              </Button>
              <Button
                variant={apostaEspelhoFilter === true ? "default" : "outline"}
                onClick={() => setApostaEspelhoFilter(true)}
                className={`${
                  apostaEspelhoFilter === true 
                    ? "bg-orange-600 hover:bg-orange-500 text-white" 
                    : "border-bolao-card-border hover:border-orange-500/50"
                }`}
              >
              ...
              <Button
                variant={apostaEspelhoFilter === false ? "default" : "outline"}
                onClick={() => setApostaEspelhoFilter(false)}
                className={`${
                  apostaEspelhoFilter === false 
                    ? "bg-orange-600 hover:bg-orange-500 text-white" 
                    : "border-bolao-card-border hover:border-orange-500/50"
                }`}
              >
          ...
          {/* Teimosinha Filter */}
          <div>
            <p className="text-sm font-medium text-muted-foreground mb-3">Filtrar por Teimosinha:</p>
            <div className="flex flex-wrap gap-2">
              <Button
                variant={teimosinhaModeFilter === null ? "default" : "outline"}
                onClick={() => setTeimosinhaModeFilter(null)}
                className={`${
                  teimosinhaModeFilter === null 
                    ? "bg-purple-600 hover:bg-purple-500 text-white" 
                    : "border-bolao-card-border hover:border-purple-500/50"
                }`}
              >
                Todos
              </Button>
              <Button
                variant={teimosinhaModeFilter === "ate_ganhar" ? "default" : "outline"}
                onClick={() => setTeimosinhaModeFilter("ate_ganhar")}
                className={`${
                  teimosinhaModeFilter === "ate_ganhar" 
                    ? "bg-purple-600 hover:bg-purple-500 text-white" 
                    : "border-bolao-card-border hover:border-purple-500/50"
                }`}
              >
              ...
              <Button
                variant={teimosinhaModeFilter === "quantidade" ? "default" : "outline"}
                onClick={() => setTeimosinhaModeFilter("quantidade")}
                className={`${
                  teimosinhaModeFilter === "quantidade" 
                    ? "bg-purple-600 hover:bg-purple-500 text-white" 
                    : "border-bolao-card-border hover:border-purple-500/50"
                }`}
              >
