...
const CreateBolaoModal = ({ isOpen, onClose, onCreateBolao }: { 
  ...
  const [apostaEspelhoEnabled, setApostaEspelhoEnabled] = useState(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <Card className={`relative z-10 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto ${isDark ? "bg-[#111827] border-[#1C2432]" : "bg-white border-gray-200"}`}>
        ...
        <div className="p-5 space-y-5">
          ...
          {/* Aposta Espelho Section - Only for Lotomania */}
          {selectedType === "lotomania" && (
            <div className={`p-4 rounded-xl border ${apostaEspelhoEnabled ? "border-orange-500/50 bg-orange-500/5" : isDark ? "border-[#1C2432]" : "border-gray-200"}`}>
              <label className="flex items-start gap-3 cursor-pointer">
                <input 
                  type="checkbox" 
                  checked={apostaEspelhoEnabled}
                  onChange={(e) => setApostaEspelhoEnabled(e.target.checked)}
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <p className={`font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>Aposta Espelho</p>
                    <Badge className="bg-orange-500/10 text-orange-400 border-orange-500/30 text-xs">
                      Lotomania
                    </Badge>
                  </div>
                  <p className={`text-xs mt-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>
                    Gera automaticamente uma aposta com as 50 dezenas restantes
                  </p>
                </div>
              </label>

              {apostaEspelhoEnabled && (
                <div className="mt-4 ml-8 space-y-3">
                  {/* Explanation Card */}
                  <div className={`p-3 rounded-lg ${isDark ? "bg-orange-500/5" : "bg-orange-50"} border border-orange-500/20`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className={`text-sm font-medium ${isDark ? "text-orange-300" : "text-orange-700"}`}>
                          Como funciona a Aposta Espelho?
                        </p>
                        <ul className={`text-xs mt-2 space-y-1 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                          <li>• Lotomania possui 100 números (00 a 99)</li>
                          <li>• Uma aposta usa 50 dezenas escolhidas</li>
                          <li>• A <span className="font-semibold text-orange-400">Aposta Espelho</span> usa as outras 50 dezenas automaticamente</li>
                          <li>• Dobra as chances de premiação com cobertura total</li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Visual Example */}
                  <div className={`p-3 rounded-lg border ${isDark ? "border-orange-500/30 bg-[#0A0E14]" : "border-orange-200 bg-white"}`}>
                    <p className={`text-xs font-medium mb-2 ${isDark ? "text-gray-300" : "text-gray-700"}`}>Exemplo:</p>
                    <div className="flex items-center gap-3">
                      <div className={`flex-1 p-2 rounded-lg text-center ${isDark ? "bg-orange-500/10" : "bg-orange-50"}`}>
                        <p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-400" : "text-gray-500"}`}>Aposta Principal</p>
                        <p className="text-sm font-bold text-orange-400">00-49</p>
                        <p className={`text-[10px] ${isDark ? "text-gray-500" : "text-gray-400"}`}>50 dezenas</p>
                      </div>
                      <ArrowRight className="w-4 h-4 text-orange-400" />
                      <div className={`flex-1 p-2 rounded-lg text-center ${isDark ? "bg-orange-500/10" : "bg-orange-50"}`}>
                        <p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-400" : "text-gray-500"}`}>Aposta Espelho</p>
                        <p className="text-sm font-bold text-orange-400">50-99</p>
                        <p className={`text-[10px] ${isDark ? "text-gray-500" : "text-gray-400"}`}>50 dezenas</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className={`p-3 rounded-lg ${isDark ? "bg-orange-500/5" : "bg-orange-50"} border border-orange-500/20`}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-medium ${isDark ? "text-orange-300" : "text-orange-700"}`}>
                        🔀 Aposta Espelho Ativa - Sistema gerará automaticamente a aposta complementar
                      </span>
                    </div>
                  </div>
                </div>
...
const EditBolaoModal = ({ 
  ...
  // Aposta Espelho state (Lotomania only)
  const [apostaEspelhoEnabled, setApostaEspelhoEnabled] = useState(false);
  ...
  useMemo(() => {
    if (bolao) {
      ...
      // Default notify for special events
      setNotifyParticipants(isSpecialEventBolao(bolao));
      // Initialize aposta espelho
      setApostaEspelhoEnabled(bolao.apostaEspelho?.enabled || false);
    }
  }, [bolao]);
  ...
  const calculateChanges = (): FieldChange[] => {
    ...
    // Track apostaEspelho changes (Lotomania only)
    if (bolao.type === "lotomania") {
      const oldApostaEspelho = bolao.apostaEspelho?.enabled || false;
      if (apostaEspelhoEnabled !== oldApostaEspelho) {
        changes.push({
          field: "apostaEspelho",
          fieldLabel: "Aposta Espelho",
          oldValue: oldApostaEspelho ? "Ativado" : "Desativado",
          newValue: apostaEspelhoEnabled ? "Ativado" : "Desativado",
        });
      }
    }
  ...
  const handleSave = () => {
    ...
    const updatedBolao: Bolao = {
      ...
      horarioEncerramento,
      status,
      // Include apostaEspelho for Lotomania
      ...(bolao.type === "lotomania" && {
        apostaEspelho: { enabled: apostaEspelhoEnabled }
      }),
    };
